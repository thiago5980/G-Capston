...
from capston_msgs.srv import Waypoints
...
class BasicNavigator(Node):
    ...
    def __init__(self, node_name='basic_navigator', namespace=''):
        ...
        self.get_waypoints_srv = self.create_service(Waypoints, '/ccpp_waypoints', self.get_waypoints_callback)
    ...
    def get_waypoints_callback(self, request, response):
        self.waypointArray = request.points
        print(self.waypointArray)
        response.getpoint = True
        self.getStart = True
        return response

    def waitWaypoints(self):
        while not self.getStart:
            self.info('wait for get waypoints')
            rclpy.spin_once(self, timeout_sec=0.5)
        self.info('received waypoints')
        return
    ...
    def rotate_to_goal(self, pos1, pos2):
        vec1 = np.array([pos2.x - pos1.x, pos2.y - pos1.y])
        unit_vector_x = np.array([1, 0])
        if (np.linalg.norm(vec1)< 0.001): return 0.0
        cosine_angle = np.dot(vec1, unit_vector_x) / np.linalg.norm(vec1)
    ...
    def rotate_quarternion(self, q, angle):
        rotate_q = quaternion_from_euler(0, 0, angle)
        result_quarterion = quaternion_multiply([q.x, q.y, q.z, q.w], rotate_q)
        updated_quaternion = Quaternion()
        updated_quaternion.x = result_quarterion[0]
        updated_quaternion.y = result_quarterion[1]
        updated_quaternion.z = result_quarterion[2]
        updated_quaternion.w = result_quarterion[3]
        return updated_quaternion
    
    def make_goal_pose(self, initial_pose):
        angle = self.quaternion_to_yaw(initial_pose.pose.orientation)
        orientation = initial_pose.pose.orientation
        goal_poses = []
        
        for i, pose in enumerate(self.waypointArray.poses):
            goal_pose = PoseStamped()
            goal_pose.header.frame_id = 'map'
            goal_pose.header.stamp = self.get_clock().now().to_msg()
            goal_pose.pose.position.x = pose.position.x
            goal_pose.pose.position.y = pose.position.y
            if i+1<len(self.waypointArray.poses):
                goal_angle = self.rotate_to_goal(pose.position, self.waypointArray.poses[i+1].position)                
                rot_angle = math.radians(goal_angle - angle)
                # goal_pose.pose.orientation = self.rotate_quarternion(orientation, rot_angle)
                goal_pose.pose.orientation = self.initial_pose.pose.orientation 
